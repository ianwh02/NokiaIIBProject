function [RI,PMISet] = hRISelect(carrier,csirs,dmrsConfig,reportConfig,H,varargin)
% hRISelect Rank indicator calculation
%   [RI,PMISET] = hRISelect(CARRIER,CSIRS,DMRSCONFIG,REPORTCONFIG,H)
%   returns the downlink channel rank indicator (RI) value RI and
%   corresponding precoding matrix indicator (PMI) values PMISET, as
%   defined in TS 38.214 Section 5.2.2.2, for the specified carrier
%   configuration CARRIER, CSI-RS configuration CSIRS,  for the specified
%   demodulation reference signal (DM-RS) configuration object for the
%   physical downlink shared channel (PDSCH), channel state information
%   (CSI) reporting configuration REPORTCONFIG, and estimated channel
%   information H.
%   
...
%
%   [RI,PMISET] = hRISelect(CARRIER,CSIRS,DMRSCONFIG,REPORTCONFIG,H,NVAR)
...
%   Copyright 2021-2024 The MathWorks, Inc.
    ...
    [reportConfig,csirsInd,nVar,alg] = validateInputs(carrier,csirs,dmrsConfig,reportConfig,H,nVar,alg);
    ...
    if ~isempty(validRanks) && ~isempty(csirsInd)
        if strcmpi(alg,'MaxSINR')
            ...
            [RI,PMISet] = riSelectCQI(carrier,csirs,dmrsConfig,reportConfig,H,nVar,validRanks,PMISubbandInfo);
        end
    end
end
...
function [RI,PMISet] = riSelectCQI(carrier,csirs,dmrsConfig,reportConfig,H,nVar,validRanks,PMISubbandInfo)
    ...
    for rank = validRanks
        ...
        [cqi,pmi(rank),cqiInfo] = hCQISelect(carrier,csirs,dmrsConfig,reportConfig,rank,H,nVar); %#ok<AGROW>
...
function [reportConfig,csirsInd,nVar,alg] = validateInputs(carrier,csirs,dmrsConfig,reportConfig,H,nVar,alg)
...
%   CSIRSIND, the noise variance NVAR, algorithm ALG
