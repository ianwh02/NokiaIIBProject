...
%   [CQI,PMISET,CQIINFO,PMIINFO] = hCQISelect(CARRIER,CSIRS,DMRSCONFIG,REPORTCONFIG,NLAYERS,H)
...
%   CSIRS, demodulation reference signal (DM-RS) configuration for physical
%   downlink shared channel DMRSCONFIG, channel state information (CSI)
...
%
%   [CQI,PMISET,CQIINFO,PMIINFO] = hCQISelect(CARRIER,CSIRS,DMRSCONFIG,REPORTCONFIG,NLAYERS,H,NVAR)
...
%
%   [CQI,PMISET,CQIINFO,PMIINFO] = hCQISelect(CARRIER,CSIRS,DMRSCONFIG,REPORTCONFIG,NLAYERS,H,NVAR,SINRTABLE)
...
%
%   [CQI,PMISET,CQIINFO,PMIINFO] = hCQISelect(CARRIER,DMRSCONFIG,BWP,CQICONFIG,CSIRSIND,H,NVAR) 
...
%   Copyright 2019-2024 The MathWorks, Inc.
    ...
    [reportConfig,nLayers,H,nVar,SINRTable,isCSIRSObjSyntax,nTxAnts,csirsInd,csirs,dmrsConfig,inputSINRTable] = parseInputs(carrier,varargin);
    ...
    if ~inputSINRTable
        ...
        [pdsch,pdschExt] = hCSIReferenceResource(carrier,reportConfig,nLayers,dmrsConfig);
...
function [reportConfig,nLayers,H,nVar,SINRTable,isCSIRSObjSyntax,nTxAnts,csirsInd,csirs,dmrsConfig,inputSINRTable] = parseInputs(carrier,varargin)
%   [REPORTCONFIG,NLAYERS,H,NVAR,SINRTABLE,ISCSIRSOBJSYNTAX,NTXANTS,CSIRSIND,CSIRS,DMRSCONFIG] = parseInputs(CARRIER,CSIRS,DMRSCONFIG,REPORTCONFIG,NLAYERS,H,NVAR,SINRTABLE) 
...
%   DMRSCONFIG   - PDSCH DM-RS configuration object
...
%
%   [REPORTCONFIG,NLAYERS,H,NVAR,SINRTABLE,ISCSIRSOBJSYNTAX,NTXANTS,CSIRSIND,CSIRS] = parseInputs(CARRIER,DMRSCONFIG,BWP,CQICONFIG,CSIRSIND,H,NVAR) 
%   returns the parsed arguments and other required parameters for the
...
%   NVAR         - Estimated noise variance
    ...
    if isstruct(variableInputArgs{2})
        ...
        % Extract PDSCH DM-RS configuration from second variable input
        % argument
        dmrsConfig = variableInputArgs{1};
        
        % Extract bwp from the first variable input argument
        bwp = variableInputArgs{2};
        ...
        reportConfig = variableInputArgs{3};
    ...
    elseif isa(variableInputArgs{1},'nrCSIRSConfig')
        ...
        % Extract the PDSCH DM-RS configuration object as dmrsConfig from
        % the second variable input argument
        dmrsConfig = variableInputArgs{2};
        ...
        % Extract the noise variance, DM-RS configuration and SINR lookup
        % table
        nVar = 1e-10;
        SINRTable = [];

        if numVarInputArgs == 6
            nVar = variableInputArgs{6};
        elseif numVarInputArgs == 7
            nVar = variableInputArgs{6};
            SINRTable = variableInputArgs{7};
        end
    ...
    % DM-RS configuration is not required when SINRTables are present,
    % validate DM-RS config when SINRTable is empty
    if ~inputSINRTable
        validateattributes(dmrsConfig,{'nrPDSCHDMRSConfig'},{'scalar'},'hCQISelect','DMRSCONFIG');
    end
...
function [pdsch,pdschExt] = hCSIReferenceResource(carrier,reportConfig,numLayers,dmrsConfig)
