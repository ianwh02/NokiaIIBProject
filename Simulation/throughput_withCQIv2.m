...
simParameters.SNRIn = [-5 20]; % SNR range (dB)
...
simParameters.PDSCH.DMRS.DMRSPortSet          = 0:1;

simParameters.CSIRS = nrCSIRSConfig;
simParameters.CSIRS.CSIRSType = 'nzp'; % 'nzp','zp'
simParameters.CSIRS.RowNumber = 4; %4 for 4 ports, 6 for 8 ports
...
simParameters.CSIRS.SubcarrierLocations = 0; %[0, 3, 6, 9], or 0
...
% statement and uncomment the 'parfor' statement.
    ...
    for nslot = 0:NSlots-1
        ...
        if isNewReport
            ...
            info = hDLPMISubbandInfo(carrier, simParameters.CSIReportConfig);
            bandset = info.SubbandSet.';
            nBands = max(bandset);

            Wband = generatePMIPrecoders(simParameters.CSIReportConfig.PMIMode, [], simParameters.NTxAnts, pdsch.NumLayers, nBands);
        ...
        portsUsed = pdsch.DMRS.DMRSPortSet + 1;
        Wband_sub =  Wband( portsUsed, :, : );
        wtx = hPRGPrecoders(carrier, pdschextra.PRGBundleSize, Wband_sub, bandset);
