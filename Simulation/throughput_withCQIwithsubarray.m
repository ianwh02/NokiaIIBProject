...
simParameters.SNRIn = 5:5:20; % SNR range (dB)
...
% pick the right full-array size so that you end up
% grouping into 32-element chunks per port:
if simParameters.CSIRS.NumCSIRSPorts==4
    simParameters.TransmitAntennaArray.Size = [8 2 2 8 1];
elseif simParameters.CSIRS.NumCSIRSPorts==8
    simParameters.TransmitAntennaArray.Size = [8 4 2 8 1];
else
    error('Unsupported number of ports');
end

% Pull out the full Size vector
fullSize   = simParameters.TransmitAntennaArray.Size;    % [nRows nCols nPol nRowPanels nColPanels]
numPorts   = simParameters.CSIRS.NumCSIRSPorts;          % 4 or 8
numPhysEls = prod(fullSize);                             % total radiators

% Figure out which element lives in which panel
elsPerPanel = fullSize(1)*fullSize(2)*fullSize(3);       % nRows×nCols×nPol
el2panel    = ceil((1:numPhysEls)/elsPerPanel);         % panel index for each element

% How many panels go to each port?
panelsTotal   = fullSize(4)*fullSize(5);                 % nRowPanels×nColPanels
panelsPerPort = panelsTotal/numPorts;
assert(mod(panelsPerPort,1)==0, 'Panels must split evenly');

% Build the boolean map E(element,port)
E = false(numPhysEls, numPorts);
for pIdx = 1:numPorts
    blocks = (pIdx-1)*panelsPerPort + (1:panelsPerPort);
    sel    = ismember(el2panel, blocks);
    E(sel, pIdx) = true;
end

% Save it in simParameters for later use
simParameters.SubarrayMap = E;

% Since we’ve collapsed our transmit “elements” to ports, 
% tell the CSIRS validator how many logical TX antennas we have:
simParameters.NTxAnts = numPorts;
...
% statement and uncomment the 'parfor' statement.
    ...
    % 2) Pull out the OFDM channel response directly:
    %    Hofdm: [Nsc × Nsym × Nr × Ntx] by default
    [Hofdm, timingOffset] = channel(simParameters.Carrier);  
    
    % 3) Permute to [Nr × Ntx × Nsc × Nsym] for our processing:
    Hphys = permute(Hofdm, [3 4 1 2]);

    % Hphys has size [Nr x numPhysEls x Nsc x Nsym]
    % simParameters.SubarrayMap is a logical matrix E of size [numPhysEls x numPorts]
    
    E        = simParameters.SubarrayMap;
    numPorts = size(E,2);
    
    [Nr, numPhysEls, Nsc, Nsym] = size(Hphys);
    Hport = zeros(Nr, numPorts, Nsc, Nsym);
    
    % Collapse by averaging all elements in each sub-array (port)
    for p = 1:numPorts
        els = find(E(:,p));          % indices of elements in port p
        % initialize accumulator for this port
        acc = zeros(Nr, 1, Nsc, Nsym);
        % sum across those elements
        for k = 1:numel(els)
            acc = acc + Hphys(:, els(k), :, :);
        end
        % average
    ...
    % Now Hport is [Nr x numPorts x Nsc x Nsym] and ready for CSI-RS codebook:
    [Fport, pmi] = nrCodebookPrecoder(Hport, simParameters.CSIReportConfig, simParameters.PDSCH.NumLayers);
    Fphys = simParameters.SubarrayMap * Fport;
...
% createChannel  Build an nrCDLChannel in subarray (port) mode
%
%   channel = createChannel(simParameters) returns a configured
%   nrCDLChannel where:
%     - channel.NumTransmitAntennas = number of CSI-RS ports
%     - Transmit/Receive arrays are taken straight from simParameters
%     - LOS orientation is computed from TX/RX 3D positions

    % Unpack a few things
    nRxAnts = simParameters.NRxAnts;
    numPorts = simParameters.CSIRS.NumCSIRSPorts;

    % Only support CDL here
    if ~contains(simParameters.DelayProfile,'CDL')
        error('This helper only configures CDL channels.');
    end
    
    channel = nrCDLChannel;
    channel.ChannelFiltering     = false;                 % disable waveform filtering
    channel.ChannelResponseOutput = 'ofdm-response';      % return OFDM response

    %% 2) Assign your full panel‐array objects
    %    (These already have Size, ElementSpacing,
    %     PolarizationAngles, Orientation, Element, etc. set)
    channel.TransmitAntennaArray = simParameters.TransmitAntennaArray;
    channel.ReceiveAntennaArray  = simParameters.ReceiveAntennaArray;


    %% 5) Common CDL settings
